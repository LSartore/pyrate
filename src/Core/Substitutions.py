        ...
        for k,v in substitutions.items():
            ...
            if k in self.allCouplings:
                ...
                if case(k, v, 'rename') and not case(k, v, 'zero') and not case(k, v, 'diag'):
                    ...
                    # TEST : add new couplings to self.allCouplings
            ...
            if (k not in self.allCouplings and case(k, v, 'sub')) or selfSub:
                ...
                if coupling == symb and cType == 'GaugeCouplings':
                    ...
                    # self.gutNorm[coupling] = (symb, expr, coupling/aux)
                ...
                # invert = solveset(symb - expr, coupling).args
                
                # # if len(invert) > 1 and cType == 'GaugeCouplings':
                # if len(invert) == 2 and invert[0] == -1*invert[1]:
                #     invert = [sol for sol in invert if all([arg.is_negative != True for arg in sol.args])]
                # if len(invert) > 1:
                #     loggingCritical(f"Warning : inverting the substitution relation < {k} : {v} > gives more than one solution. Skipping.")
                #     continue
                # else:
                #     invert = invert[0]
...
def doSubstitutions(self, substitutionDic, inconsistentRGEerror=False):
    # TODO ? : Add an option to switch on/off the following
    # -> I guess this should be seen as a feature instead of an option
            ...
            for cType, loopDic in self.couplingRGEs.items():
                for nLoop, RGEdic in loopDic.items():
                    if cType == v[0] and k in RGEdic:
                        ...
                        # self.couplingRGEs[v[0]][nLoop] = {(c if c != k else v[1]):bFunc for c,bFunc in RGEdic.items()}
    ...
    if self.gutNorm != {}:
        # loggingInfo("\t Applying GUT normalization ...")
    ...
    if 'yukMat' in substitutionDic and substitutionDic['yukMat'] != {}:
        # loggingInfo("\t Substituting Yukawa matrices...")
        ...
        def traceSub(tr):
            ...
            if identitySubs != {}:
                # print("\n\n\n ## IDENTITY SUBS : ", identitySubs)
                # print(tr)
                # print(tr.args[0].atoms())
    ...
    if 'sub' in substitutionDic and substitutionDic['sub'] != {}:
        # loggingInfo("\t Substituting couplings ...")
