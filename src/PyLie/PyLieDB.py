...
class PyLieDB():
    ...
    def __init__(self, path=None, logLevel='Info', raiseErrors=False):
        ...
        self.storedTranslations = {
            # 'invariants' : lambda a, reps, kwargs: a.invariants(reps, **kwargs),
    ...
    def load(self, force=False):
        ...
        if created:
            # self.f = h5py.File(self.tmpPath, 'a')
            # print("File was created", created)
    ...
    def loggingCritical(self, mess):
        if self.logging['critical'] and not self.raiseErrors:
            print(mess)
        if self.raiseErrors:
            # raise BaseException("An error occured while interacting with the DB.")
    ...
    def convert(val, objType=None, allStr=False, dataGroup=None, objName=None):
        ...
        # print(f"\tConvert with val of type {type(val)}")
        # print(isinstance(val, Rational))
        ...
        if isinstance(val, list) or isinstance(val, tuple):
            # return [PyLieDB.convert(el, allStr=allStr,
            #                             dataGroup=dataGroup,
            #                             objName=objName) for el in val]
            return [PyLieDB.convert(el) for el in val]
        
        # # a**b
        # if isinstance(val, Pow):
        #     return 'p('+PyLieDB.convert(val.args[0])+','+PyLieDB.convert(val.args[1])+')'
        
        # # Rational
        # if isinstance(val, Rational):
        #     n,d = val.numerator(), val.denominator()
        #     if d != 1:
        #         return 'r('+PyLieDB.convert(n)+','+PyLieDB.convert(d)+')'
        #     else:
        #         return PyLieDB.convert(n)
    ...
    def handleInput(self, algebra, dataType, args, kwargs):
        ...
        if dataType == 'invariants':
            ...
            # print("Invariants storeName : ", storeName)
        ...
        if dataType == 'repmatrices':
            ...
            if len(arg) != algebra._n:
                # self.loggingCritical(f"Error in 'repMatrices' : {arg} is not a valid representation of {algebra.fn}.")
