...
yd = lambda i,j : P.__getattribute__('yd%dx%d' % (i+1, j+1))
yl = lambda i,j : P.__getattribute__('yl%dx%d' % (i+1, j+1))
yu = lambda i,j : P.__getattribute__('yu%dx%d' % (i+1, j+1))
...
def generateYukTerm(couplings, indices, ranges, conj=()):
    ...
    indsProduct = itertools.product(*[range(r) for r in ranges])
    return [couplings[0] + [conjugate(cPos)(c(pInd[i[0]], pInd[i[1]])) for cPos, (c, i) in enumerate(zip(couplings[1], indices))] + couplings[2] for pInd in indsProduct] 
    
cc = lambda x: conjugate(x)

gg= generateYukTerm([[P.muH], [yl, yl], [P.muH]], [(0, 1), (0, 1)], (3, 3), (1,))




ll = [
                                       *generateYukTerm([[P.muH], [yd, yd], [P.muH]], [(0, 1), (0, 1)], (3, 3)),
                                       *generateYukTerm([[P.muH], [yu, yu], [P.muH]], [(0, 1), (0, 1)], (3, 3))
]