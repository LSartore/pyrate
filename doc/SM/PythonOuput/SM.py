...
class RGEsolver():
    ...
    def __init__(self, name, initialScale = 0, tmin = 0, tmax = 20):
        ...
        # Gauge Couplings
        self.g_1 = Coupling('g_1', 'GaugeCouplings')
        self.g_2 = Coupling('g_2', 'GaugeCouplings')
        self.g_3 = Coupling('g_3', 'GaugeCouplings')
    ...
    def betaFunction(self, t, couplingsArray):
        ...
        g_1, g_2, g_3 = self.extractCouplings(couplingsArray, 'GaugeCouplings')
        ...
        if self.loops['GaugeCouplings'] >= 1:
            bg_1 += ((41/10)*g_1**3)*self.kappa(1)*np.log(10)
            bg_2 += (-19/6*g_2**3)*self.kappa(1)*np.log(10)
            bg_3 += (-7*g_3**3)*self.kappa(1)*np.log(10)
        if self.loops['GaugeCouplings'] >= 2:
            bg_1 += ((199/50)*g_1**5 + (27/10)*g_1**3*g_2**2 + (44/5)*g_1**3*g_3**2 - 1/2*g_1**3*abs(yb)**2 - 17/10*g_1**3*abs(yt)**2 - 3/2*g_1**3*abs(ytau)**2)*self.kappa(2)**2*np.log(10)
            bg_2 += ((9/10)*g_1**2*g_2**3 + (35/6)*g_2**5 + 12*g_2**3*g_3**2 - 3/2*g_2**3*abs(yb)**2 - 3/2*g_2**3*abs(yt)**2 - 1/2*g_2**3*abs(ytau)**2)*self.kappa(2)**2*np.log(10)
            bg_3 += ((11/10)*g_1**2*g_3**3 + (9/2)*g_2**2*g_3**3 - 26*g_3**5 - 2*g_3**3*abs(yb)**2 - 2*g_3**3*abs(yt)**2)*self.kappa(2)**2*np.log(10)

        if self.loops['Yukawas'] >= 1:
            byt += (-17/20*g_1**2*yt - 9/4*g_2**2*yt - 8*g_3**2*yt + (3/2)*yt*abs(yb)**2 + (9/2)*yt*abs(yt)**2 + yt*abs(ytau)**2)*self.kappa(1)*np.log(10)
            byb += (-1/4*g_1**2*yb - 9/4*g_2**2*yb - 8*g_3**2*yb + (9/2)*yb*abs(yb)**2 + (3/2)*yb*abs(yt)**2 + yb*abs(ytau)**2)*self.kappa(1)*np.log(10)
            bytau += (-9/4*g_1**2*ytau - 9/4*g_2**2*ytau + 3*ytau*abs(yb)**2 + 3*ytau*abs(yt)**2 + (5/2)*ytau*abs(ytau)**2)*self.kappa(1)*np.log(10)
        if self.loops['Yukawas'] >= 2:
            byt += ((1187/600)*g_1**4*yt - 9/20*g_1**2*g_2**2*yt + (19/15)*g_1**2*g_3**2*yt + (7/80)*g_1**2*yt*abs(yb)**2 + (393/80)*g_1**2*yt*abs(yt)**2 + (15/8)*g_1**2*yt*abs(ytau)**2 - 23/4*g_2**4*yt + 9*g_2**2*g_3**2*yt + (99/16)*g_2**2*yt*abs(yb)**2 + (225/16)*g_2**2*yt*abs(yt)**2 + (15/8)*g_2**2*yt*abs(ytau)**2 - 108*g_3**4*yt + 4*g_3**2*yt*abs(yb)**2 + 36*g_3**2*yt*abs(yt)**2 + 6*lambda_**2*yt - 12*lambda_*yt*abs(yt)**2 - 1/4*yt*abs(yb)**4 - 11/4*yt*abs(yb)**2*abs(yt)**2 + (5/4)*yt*abs(yb)**2*abs(ytau)**2 - 12*yt*abs(yt)**4 - 9/4*yt*abs(yt)**2*abs(ytau)**2 - 9/4*yt*abs(ytau)**4)*self.kappa(2)**2*np.log(10)
            byb += (-127/600*g_1**4*yb - 27/20*g_1**2*g_2**2*yb + (31/15)*g_1**2*g_3**2*yb + (237/80)*g_1**2*yb*abs(yb)**2 + (91/80)*g_1**2*yb*abs(yt)**2 + (15/8)*g_1**2*yb*abs(ytau)**2 - 23/4*g_2**4*yb + 9*g_2**2*g_3**2*yb + (225/16)*g_2**2*yb*abs(yb)**2 + (99/16)*g_2**2*yb*abs(yt)**2 + (15/8)*g_2**2*yb*abs(ytau)**2 - 108*g_3**4*yb + 36*g_3**2*yb*abs(yb)**2 + 4*g_3**2*yb*abs(yt)**2 + 6*lambda_**2*yb - 12*lambda_*yb*abs(yb)**2 - 12*yb*abs(yb)**4 - 11/4*yb*abs(yb)**2*abs(yt)**2 - 9/4*yb*abs(yb)**2*abs(ytau)**2 - 1/4*yb*abs(yt)**4 + (5/4)*yb*abs(yt)**2*abs(ytau)**2 - 9/4*yb*abs(ytau)**4)*self.kappa(2)**2*np.log(10)
            bytau += ((1371/200)*g_1**4*ytau + (27/20)*g_1**2*g_2**2*ytau + (5/8)*g_1**2*ytau*abs(yb)**2 + (17/8)*g_1**2*ytau*abs(yt)**2 + (537/80)*g_1**2*ytau*abs(ytau)**2 - 23/4*g_2**4*ytau + (45/8)*g_2**2*ytau*abs(yb)**2 + (45/8)*g_2**2*ytau*abs(yt)**2 + (165/16)*g_2**2*ytau*abs(ytau)**2 + 20*g_3**2*ytau*abs(yb)**2 + 20*g_3**2*ytau*abs(yt)**2 + 6*lambda_**2*ytau - 12*lambda_*ytau*abs(ytau)**2 - 27/4*ytau*abs(yb)**4 + (3/2)*ytau*abs(yb)**2*abs(yt)**2 - 27/4*ytau*abs(yb)**2*abs(ytau)**2 - 27/4*ytau*abs(yt)**4 - 27/4*ytau*abs(yt)**2*abs(ytau)**2 - 3*ytau*abs(ytau)**4)*self.kappa(2)**2*np.log(10)

        if self.loops['QuarticTerms'] >= 1:
            blambda_ += ((27/200)*g_1**4 + (9/20)*g_1**2*g_2**2 - 9/5*g_1**2*lambda_ + (9/8)*g_2**4 - 9*g_2**2*lambda_ + 24*lambda_**2 + 12*lambda_*abs(yb)**2 + 12*lambda_*abs(yt)**2 + 4*lambda_*abs(ytau)**2 - 6*abs(yb)**4 - 6*abs(yt)**4 - 2*abs(ytau)**4)*self.kappa(1)*np.log(10)
        if self.loops['QuarticTerms'] >= 2:
            blambda_ += (-3411/2000*g_1**6 - 1677/400*g_1**4*g_2**2 + (1887/200)*g_1**4*lambda_ + (9/20)*g_1**4*abs(yb)**2 - 171/100*g_1**4*abs(yt)**2 - 9/4*g_1**4*abs(ytau)**2 - 289/80*g_1**2*g_2**4 + (117/20)*g_1**2*g_2**2*lambda_ + (27/10)*g_1**2*g_2**2*abs(yb)**2 + (63/10)*g_1**2*g_2**2*abs(yt)**2 + (33/10)*g_1**2*g_2**2*abs(ytau)**2 + (108/5)*g_1**2*lambda_**2 + (5/2)*g_1**2*lambda_*abs(yb)**2 + (17/2)*g_1**2*lambda_*abs(yt)**2 + (15/2)*g_1**2*lambda_*abs(ytau)**2 + (4/5)*g_1**2*abs(yb)**4 - 8/5*g_1**2*abs(yt)**4 - 12/5*g_1**2*abs(ytau)**4 + (305/16)*g_2**6 - 73/8*g_2**4*lambda_ - 9/4*g_2**4*abs(yb)**2 - 9/4*g_2**4*abs(yt)**2 - 3/4*g_2**4*abs(ytau)**2 + 108*g_2**2*lambda_**2 + (45/2)*g_2**2*lambda_*abs(yb)**2 + (45/2)*g_2**2*lambda_*abs(yt)**2 + (15/2)*g_2**2*lambda_*abs(ytau)**2 + 80*g_3**2*lambda_*abs(yb)**2 + 80*g_3**2*lambda_*abs(yt)**2 - 32*g_3**2*abs(yb)**4 - 32*g_3**2*abs(yt)**4 - 312*lambda_**3 - 144*lambda_**2*abs(yb)**2 - 144*lambda_**2*abs(yt)**2 - 48*lambda_**2*abs(ytau)**2 - 3*lambda_*abs(yb)**4 - 42*lambda_*abs(yb)**2*abs(yt)**2 - 3*lambda_*abs(yt)**4 - lambda_*abs(ytau)**4 + 30*abs(yb)**6 - 6*abs(yb)**4*abs(yt)**2 - 6*abs(yb)**2*abs(yt)**4 + 30*abs(yt)**6 + 10*abs(ytau)**6)*self.kappa(2)**2*np.log(10)

        if self.loops['ScalarMasses'] >= 1:
            bmu += (-9/10*g_1**2*mu - 9/2*g_2**2*mu + 12*lambda_*mu + 6*mu*abs(yb)**2 + 6*mu*abs(yt)**2 + 2*mu*abs(ytau)**2)*self.kappa(1)*np.log(10)
        if self.loops['ScalarMasses'] >= 2:
            bmu += ((1671/400)*g_1**4*mu + (9/8)*g_1**2*g_2**2*mu + (72/5)*g_1**2*lambda_*mu + (5/4)*g_1**2*mu*abs(yb)**2 + (17/4)*g_1**2*mu*abs(yt)**2 + (15/4)*g_1**2*mu*abs(ytau)**2 - 145/16*g_2**4*mu + 72*g_2**2*lambda_*mu + (45/4)*g_2**2*mu*abs(yb)**2 + (45/4)*g_2**2*mu*abs(yt)**2 + (15/4)*g_2**2*mu*abs(ytau)**2 + 40*g_3**2*mu*abs(yb)**2 + 40*g_3**2*mu*abs(yt)**2 - 60*lambda_**2*mu - 72*lambda_*mu*abs(yb)**2 - 72*lambda_*mu*abs(yt)**2 - 24*lambda_*mu*abs(ytau)**2 - 27/2*mu*abs(yb)**4 - 21*mu*abs(yb)**2*abs(yt)**2 - 27/2*mu*abs(yt)**4 - 9/2*mu*abs(ytau)**4)*self.kappa(2)**2*np.log(10)

        if self.loops['Vevs'] >= 1:
            bv += ((3/5)*g_1**2*v + 3*g_2**2*v - 3*v*abs(yb)**2 - 3*v*abs(yt)**2 - v*abs(ytau)**2)*self.kappa(1)*np.log(10)
        if self.loops['Vevs'] >= 2:
            bv += (-1221/800*g_1**4*v + (9/16)*g_1**2*g_2**2*v - 43/40*g_1**2*v*abs(yb)**2 - 103/40*g_1**2*v*abs(yt)**2 - 81/40*g_1**2*v*abs(ytau)**2 + (199/32)*g_2**4*v - 63/8*g_2**2*v*abs(yb)**2 - 63/8*g_2**2*v*abs(yt)**2 - 21/8*g_2**2*v*abs(ytau)**2 - 20*g_3**2*v*abs(yb)**2 - 20*g_3**2*v*abs(yt)**2 - 6*lambda_**2*v + (27/4)*v*abs(yb)**4 - 3/2*v*abs(yb)**2*abs(yt)**2 + (27/4)*v*abs(yt)**4 + (9/4)*v*abs(ytau)**4)*self.kappa(2)**2*np.log(10)

        return [bg_1, bg_2, bg_3, byt, byb, bytau, blambda_, bmu, bv]
